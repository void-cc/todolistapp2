...
def submit_todo():
    ...
    if request.method == 'POST':
        ...
        if 'toevoegen' in request.form:
            ...
            job = q.enqueue(ti.add_to_todo, request.form['toevoegen'], user_id, datetime.fromisoformat(request.form['toevoegen_date']), todo_tags=request['toevoegen_tag'], todo_tags_color=request.form['toevoegen_tag_color'])
            print(job.get_id())
            if len(q) > 1:
